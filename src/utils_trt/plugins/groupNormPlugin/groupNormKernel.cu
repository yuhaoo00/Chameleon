...
struct GroupSums
{
    ...
    int32_t flag; //用于GroupSumsOp中区分不同group
    // The sum.
    float sum;
    // The sum of squares.
    float sumSq;
...
__global__ void groupNormNHWCSumKernel(GroupNormNHWCParams params)
{
    ...
    typedef cub::BlockScan<GroupSums, tTHREADS_PER_BLOCK> BlockScan; //cub并行库分块scan算法，给定shared的数据类型以及数量（单个blocksize），定义缩写
    ...
    for (int32_t hwi = hwBegin; hwi < hwEnd; ++hwi) //每个thread负责[n=1,hw=full,c=2]的sum和sumSq
    {
        // The offset.
        ...
        if (ci < params.c)
        {
            h2 = *reinterpret_cast<__half2 const*>(&params.src[offset]); //指向单个half的指针，转化为连续两个half的指针
        }
    ...
    int32_t gi = threadIdx.x * 2 / params.cPerGroup;  //block内相对的group idx
    int32_t cj = threadIdx.x * 2 - params.cPerGroup * gi; //group内相对的channel idx
    ...
    GroupSums out;
    BlockScan(tempStorage).InclusiveScan(inp, out, GroupSumsOp()); //调用scan，对block内的所有thread计算的GroupSums再相加（通过GroupSumsOp()）
    ...
    //到达group末尾时记录sum到smem
    //调用每个group末尾最后一个thread，写入局部内存smem

    // Make sure the data is in shared memory.
    __syncthreads();

    //注意这里是错位记录
    //之后，只调用每个block的前groupsPerBlock个thread，写入全局内存redBuffer
    ...
    atomicAdd(&params.redBuffer[(2 * ni + 0) * params.groups + gj], sums.x); //不同block的交互，跨hw维度相加
...
void groupNormNHWCSum(GroupNormNHWCParams const& params, cudaStream_t stream)
{
    ...
    {
    case 320: groupNormNHWCSumKernel<160><<<grid, 160, 0, stream>>>(params); break; //dynamic_shared_mem=0无需申请
    case 480: groupNormNHWCSumKernel<256><<<grid, 256, 0, stream>>>(params); break; //若240，不符合32SIMT最优设置
...
__global__ void groupNormNHWCScaleKernel(GroupNormNHWCParams params)
{
    ...
    int32_t ci = blockIdx.x * params.cPerBlock + threadIdx.x * 2;
    // The group that thread works on and the channel in the group (modulus).
    int32_t gi = ci / params.cPerGroup; // 全局的group idx
